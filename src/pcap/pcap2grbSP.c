...
void add_blob_to_tar(Entry *entries, size_t num_entries)
{
    int tarfd;
    ...
    sprintf(th.name, "%d.tup", pstate->findex);
    sprintf(th.uid, "%06o ", 0);
    sprintf(th.gid, "%06o ", 0);
    sprintf(th.size, "%011o", (unsigned int)(num_entries * sizeof(Entry)));
    ...
    if (write(tarfd, entries, num_entries * sizeof(Entry)) != (ssize_t)(num_entries * sizeof(Entry)))
    {
        perror("write tar error");
        exit(4);
    }

    aligned = (sizeof(th) + num_entries * sizeof(Entry)) % 512; // pad output file to 512 byte alignment
    if (aligned != 0)
    {
        const unsigned char padblock[512] = { 0 };

        if (write(tarfd, padblock, 512 - aligned) != (ssize_t)(512 - aligned))
        {
            perror("write tar error");
            exit(4);
        }
...
int main(int argc, char *argv[])
{
    ...
    pstate->entries          = NULL;
    pstate->entries_size     = 0;
    pstate->entries_capacity = 0;
    ...
    else if (pstate->anonymize == 2)
    {
        ...
        if ((ret_read = fread(pstate->ip4cache, filesize, 1, fp)) < 1)
        {
            perror("fread");
            return 1;
        }
    ...
    fprintf(stderr, "Generating tar files with %u windows of size: %u\n", pstate->files_per_window,
            pstate->subwinsize);

    pstate->entries_capacity = pstate->subwinsize;
    pstate->entries = malloc(sizeof(Entry) * pstate->entries_capacity);
    if (!pstate->entries) {
        ...
        if (pstate->link_type == DLT_EN10MB)
        {
            ...
            while (match != NULL) {
                ...
                if (srcip_offset + 8 <= packet_len) {
                    ...
                    if (pstate->entries_size >= pstate->entries_capacity)
                    {
                        pstate->entries_capacity *= 2;
                        pstate->entries = realloc(pstate->entries, sizeof(Entry) * pstate->entries_capacity);
                        if (!pstate->entries) {
                    ...
                    pstate->entries[pstate->entries_size].row = srcip;
                    pstate->entries[pstate->entries_size].col = dstip;
                    pstate->entries[pstate->entries_size].value = 1;

                    pstate->entries_size++;

                    if (pstate->entries_size == pstate->subwinsize)
                    {
                        add_blob_to_tar(pstate->entries, pstate->entries_size);
                        pstate->entries_size = 0;
                    }
                }
    ...
    if (pstate->entries_size > 0)
    {
        ...
        {
            fprintf(stderr, "INFO: Not processing %zu remaining entries.\n", pstate->entries_size);
        }
    }

    filepos = ftell(in);
    fprintf(stderr, "Done: %lu packets.  (%.2f pps)\n", pstate->total_packets, pstate->total_packets / t_elapsed);
    fclose(in);
    free(pstate->entries);
    free(pstate->ip4cache);
    free(pstate->out_prefix);
    free(pstate);
